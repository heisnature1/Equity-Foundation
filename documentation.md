...
- Overhauled and completed all 5 dedicated civic portal pages to fit the viewport (`min-height: calc(100svh - 84px)`) on desktop screens:
  1. **Legal Help Portal (`/legal-help` / `components/legal-help-portal.tsx`):** Viewport-fitting 2-column layout with clean ivory background illumination (removed intrusive 3D background dots that bled over text/cards), emergency hotlines card (Police 191/112, DOVVSU 0800 111 222, Legal Aid 0302 669 220), 3 streamlined trust pills (100% Free Pro Bono, Strict Confidentiality, 48-Hour Response), and an elevated SSL-encrypted pro bono triage form with prestigious emerald action button, client-side validation, and instant confirmation state.
  2. **Advocacy & Systemic Reform Portal (`/advocacy` / `components/advocacy-portal.tsx`):** Viewport-fitting showcase with 4 empirical benchmark counters (14 Divisions, 24 Townhalls, 16 Regions, 100% Non-Partisan), 3 interactive campaign cards, deep-dive modal viewer with field outcomes, and public-interest memo intake.
  3. **Resources & Handbooks Library (`/resources` / `components/resources-portal.tsx`):** Digital open-access library with live instant search, category filter pills (Rights Handbooks, Empirical Research, Toolkits & Compliance), document download triggers, and community educational materials request banner.
  4. **Support & Civic Partnership Portal (`/support` / `components/support-portal.tsx`):** Deep obsidian viewport portal (`#08090d`) with ambient gold/emerald glows, 3 verified institutional donation channels (Bank Wire with 1-click copy, Mobile Money MoMo merchant copy, In-Kind Fellowships), and an anti-fraud security guarantee.
  5. **Contact & Office Directory (`/contact` / `components/contact-portal.tsx`):** Viewport-fitting 2-column directory with headquarters address, telephone, WhatsApp, public operating hours, direct link to pro bono triage, and an elevated general inquiry form with instant submission feedback.

- **Legal Help Portal UX Clean-up & Triage Refinement:**
  - Resolved user feedback on `/legal-help`: Removed the 3D `DottedSurface` canvas from this route to prevent dots from bleeding through semi-transparent cards and obscuring emergency numbers and statutory disclaimers.
  - Replaced the bulky 3-card vertical stack with 3 streamlined trust pills (`100% Free Pro Bono`, `Strict Confidentiality`, `48-Hour Response`).
